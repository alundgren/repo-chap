...
server endpoint. Humans merge PRs.
...
Use code-guidance for technical planning and ux-design for user-facing work.
Record actual UI decisions in docs/ux.md. Test observable behavior, including
failed configuration activation, restarts, stale heads, and unknown remote
outcomes. Run relevant checks with the pinned pnpm through Corepack. Keep test
output compact and store failure diagnostics outside the checkout.
