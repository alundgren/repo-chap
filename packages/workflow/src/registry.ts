...
export interface ActionDefinition {
  ...
  invalidates?: readonly ('candidate' | 'checks' | 'push')[];
...
export const actionRegistry: Readonly<Record<string, ActionDefinition>> = freeze({
  ...
  'agent.resolve_conflict': { execution: 'agent', capabilities: ['workspace.read', 'workspace.write'], consumesAgentBudget: true, repair: true, result: 'candidate', invalidates: ['candidate', 'checks', 'push'], produces: 'candidate' },
  'agent.address_review': { execution: 'agent', capabilities: ['workspace.read', 'workspace.write'], consumesAgentBudget: true, repair: true, result: 'candidate', invalidates: ['candidate', 'checks', 'push'], produces: 'candidate' },
  'agent.classify': { execution: 'agent', capabilities: ['workspace.read'], consumesAgentBudget: true, result: 'classification' },
  'agent.review': { execution: 'agent', capabilities: ['workspace.read'], consumesAgentBudget: true, result: 'review' },
