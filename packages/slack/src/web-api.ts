...
export function validReceipt(value: unknown): value is SlackReceipt {
  return object(value) && /^T[A-Z0-9]+$/.test(String(value.workspaceId)) && /^[CGD][A-Z0-9]+$/.test(String(value.channelId)) && /^\d{1,20}\.\d{1,10}$/.test(String(value.timestamp));
}
export class SlackApi {
  ...
  async verify(signal?: AbortSignal): Promise<SlackResult<true>> {
    if (this.verified) return { status: 'confirmed', value: true };
    const response = await this.call('auth.test', {}, signal);
    ...
    this.verified = true; return { status: 'confirmed', value: true };
  }
  async openDm(memberId: string, signal?: AbortSignal): Promise<SlackResult<string>> {
    ...
    const response = await this.call('conversations.open', { users: memberId, return_im: false }, signal);
    if (response.status !== 'confirmed') return response;
  ...
  async send(channelId: string, message: SlackMessage, timestamp?: string, signal?: AbortSignal): Promise<SlackResult<SlackReceipt>> {
    ...
    if (!this.verified) throw new SlackError('Verify the installation workspace before sending a Slack message.');
    const response = await this.call(timestamp ? 'chat.update' : 'chat.postMessage', { ...message, channel: channelId, ...(timestamp ? { ts: timestamp } : {}) }, signal, channelId);
    if (response.status !== 'confirmed') return response;
  ...
  private async call(method: string, payload: Record<string, unknown>, signal?: AbortSignal, channel?: string): Promise<SlackResult<Record<string, unknown>>> {
    ...
    let token: string;
    try { token = await this.options.token(); } catch { return { status: 'rejected', reason: 'Cannot read the Slack token. Check the private installation settings.' }; }
    if (!token || /\s/.test(token)) return { status: 'rejected', reason: 'The private Slack token is missing or invalid.' };
    if (blockedUntil() > this.now()) return deferred();
    if (signal?.aborted) return { status: 'rejected', reason: 'Slack delivery was cancelled before dispatch.' };
    ...
    try {
      ...
      if (data.ok === false && (data.error === 'ratelimited' || data.error === 'rate_limited')) {
        const until = this.now() + 60_000; for (const key of keys) this.options.rates.extend(key, until);
        return { status: 'deferred', retryAt: until, attempted: true, reason: 'Slack rate limited this call. Retry after the saved delay.' };
      }
      if (typeof data.error === 'string' && rejected.has(data.error)) return { status: 'rejected', reason: `Slack rejected delivery: ${data.error}. Check the token scopes, destination and bot membership. The complete request stays in the CLI inbox.` };
      throw new Error();
