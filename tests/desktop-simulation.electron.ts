...
test('focused inspector drafts participate in save, leave protection, reset, export and simulation', { timeout: 120_000 }, async t => {
  ...
  const deadline = page.locator('#setting-reviewDeadlineSeconds');
  await time.fill('77'); await deadline.fill(''); await page.locator('#apply-settings').click();
  await expect(page.locator('#message')).toContainText('finite number'); await expect(time).toHaveValue('77'); await expect(deadline).toHaveValue(''); assert.equal((await snapshot()).workflow!.settings.reviewWaitSeconds, 77);
  await time.fill('778'); await deadline.fill('1800'); await expect(time).toHaveValue('778'); await expect(page.locator('#dirty-state')).toContainText('1 unsaved action setting(s)');
  await time.focus(); await time.press('Control+s'); await expect(page.locator('#message')).toHaveText('Saved all changed files.'); await expect(time).toHaveValue('778'); await expect(time).toBeFocused();
  assert.equal((await snapshot()).workflow!.settings.reviewWaitSeconds, 778); assert.equal((await disk()).settings.reviewWaitSeconds, 778); assert.equal((await loadWorkflow(join(root, workflowPath), { repositoryRoot: root })).workflow.settings.reviewWaitSeconds, 778);
  await time.fill('77'); await deadline.fill(''); await page.locator('#apply-settings').click(); await expect(page.locator('#message')).toContainText('finite number');
  await time.fill('778'); await time.press('End'); await time.press('Backspace'); await time.press('9'); await deadline.fill('1800'); await page.locator('#source-tab').click();
  assert.equal((await snapshot()).workflow!.settings.reviewWaitSeconds, 779); assert.equal(JSON.parse(await page.locator('#source').inputValue()).settings.reviewWaitSeconds, 779); assert.equal((await disk()).settings.reviewWaitSeconds, 778);
  await page.locator('#process-tab').click(); await expect(time).toHaveValue('779'); await time.fill('778'); await time.press('Control+s'); await expect(page.locator('#message')).toHaveText('Saved all changed files.'); assert.equal((await disk()).settings.reviewWaitSeconds, 778);
  await page.screenshot({ path: join(proof, '14-partial-settings-recovered.png'), fullPage: true });
  ...
  await writeFile(join(proof, 'inspector-result.json'), JSON.stringify({ passed: true, packaged: !!packaged, checks: ['focused numeric Ctrl+S and continued typing', 'Save button accepts focused text', 'focused prompt/context Ctrl+S', 'close/open/reset cancel retains pending field', 'reset discards pending field', 'invalid partial value retained and blocks inspector replacement', 'explicit discard', 'partial flush restoration matches field/session/reopened disk', 'partial flush continued typing survives later view capture', 'export captures pending value without changing original disk', 'simulation uses pending settings', 'save-and-open cancelled picker preserves saved value', 'save-and-close persists pending value', 'reopened package matches saved value'] }, null, 2));
});
