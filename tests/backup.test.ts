...
import { backupState, restoreState, RuntimeStore, type ApplyPolicy } from '@repo-chap/runtime';
...
test('schema 3 upgrade rejects a live prior owner before changing any persisted data', async () => {
  const s = await setup(), directory = join(s.temporary, 'state'); let store = await RuntimeStore.open(directory);
  try {
    const now = Date.now(), repo = await store.register({ id: 'R_paperboat', name: 'reef-labs/paperboat', package: s.pkg, profile: 'pilot', reviewers: [] }, now);
    const run = (await store.observe(repo.id, s.inspection, now))!, owner = store.claimDaemon(), before = store.inspect(run.id); store.close();
