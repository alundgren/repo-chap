...
async function launch(t: { after(callback: () => Promise<void>): void }, name: string) {
  ...
  for (const provider of ['codex', 'claude']) {
    ...
    await writeFile(executable, `#!${process.execPath}\nglobal.fixture=${JSON.stringify({ provider, modeFile, noTools: true, log: join(source.temporary, `${provider}-chat.jsonl`), childPid: join(source.temporary, `${provider}-chat.pid`) })};require(${JSON.stringify(resolve('tests/helpers/fake-conversation.cjs'))});`, { mode: 0o700 });
  ...
  await page.locator('#trial-tab').click(); await pick(source.settings); await page.locator('#trial-load-profiles').click();
  await page.locator('#trial-repository').fill('reef-labs/paperboat'); await page.locator('#trial-pr').fill('42'); await page.locator('#trial-profile').selectOption('codex_trial');
