...
export interface TrialProposal { document: DocumentToken; selection: TrialSelection; provider: ConversationProvider; profileDigest: string; preparedBy: 'person' | 'assistant' }
export type TrialProfile = ConversationProvider & { digest: string };
...
export interface TrialResult extends EditorResult { trial: TrialSnapshot | null; profiles: TrialProfile[] }
export interface TrialBridge {
  ...
  prepare(token: DocumentToken, selection: TrialSelection, profileDigest: string): Promise<TrialResult>;
  start(token: DocumentToken, selection: TrialSelection, profileDigest: string): Promise<TrialResult>;
  ...
  onProfilesChange(callback: (profiles: TrialProfile[]) => void): () => void;
}
